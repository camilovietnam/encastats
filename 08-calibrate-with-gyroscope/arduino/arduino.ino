...
unsigned int oneSecond = 1000; // one second
unsigned long lastTickOneSecond = 0;
...
void setup() {
  Serial.begin(9600);
  lastTickOneSecond = millis();

  MyApp.Init();

  my_gyroscope.init();
  delay(2000);
  my_gyroscope.calibrate();
...
void loop() {  
  ...
  if (now - lastTickOneSecond > oneSecond) {
    actionEverySecond();
    lastTickOneSecond = now;
  }
}
...
/* * * * * * * * * * * * * * * * * * * * * * */
/* This will take place every second   * * * */
/* * * * * * * * * * * * * * * * * * * * * * */
void actionEverySecond(void) {
  static float yaw;
  my_gyroscope.getAngle(&yaw);
  Serial.print("Yaw: ");
  Serial.println(yaw);
}